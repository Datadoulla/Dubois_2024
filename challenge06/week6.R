...
data |> add_row(
  ...
  Mulattoes = c(411613/2,0,0),
  Negroes = c(3542147*.7, 3542147*.3, 3542147*.3)
) |> mutate(
  Mulattoes = Mulattoes / 2*1.5,
  Limit = - Mulattoes,
  Year = c(1:4,4.1),
  ab = Mulattoes
) |> 
  ...
  mutate(
    ...
    name = factor(name, levels = c('Negroes', 'Mulattoes','Limit'))
    ) -> data_plot
...
data_plot |> 
  ...
  geom_segment(data=data_plot |> slice(1:12),
  ...
  labs(title = 'The Amalgamation of the White and Black elements of the population\nin the United States.',
       ...
       Done by Atlanta University, reproduice @Datadoulla.')+
  theme(
    ...
    plot.subtitle = element_text(hjust = .5, vjust = -10), 
